...
void InitScene()
{
	...
	//materials
	PxMaterial* default_material = physics->createMaterial(0.f, 0.2f, 0.8f);   //static friction, dynamic friction, restitution
	...
	box = physics->createRigidDynamic(PxTransform(PxVec3(0.f, 10.0f, 0.f)));
...
int main()
{
	...
	box->addForce(PxVec3(1000.0f, 0.0f, 0.0f)); //yeet
	...
	while (!GetAsyncKeyState(VK_ESCAPE))
	{
		...
		if ( box->isSleeping() /*priorY == position.y*/)
		{
			cout << "Stopped in " << count << " steps" << endl;	//looks like it stops late but that's just because of rounding to 2 sf
		}
		
		/*if (count == 10)
		{
			box->setGlobalPose(PxTransform(PxVec3(10.0f, position.y, position.z)));
		}*/
