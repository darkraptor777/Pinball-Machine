...
namespace VisualDebugger
{
	...
	void Init(const char *window_name, int width, int height)
	{
		...
		camera = new Camera(PxVec3(0.0f, 100.0f, 100.0f), PxVec3(0.f,-.1f,-1.f), 30.f);
	...
	void HUDInit()
	{
		...
		hud.AddLine(HELP, "");
		hud.AddLine(HELP, "");
		hud.AddLine(HELP, " Pinball Machine");
		hud.AddLine(HELP, "    Space - plunger");
		hud.AddLine(HELP, "    Z - left paddle");
		hud.AddLine(HELP, "    X - right paddle");
		...
		hud.AddLine(HELP, "    W,S,A,D,Q,E - forward,backward,left,right,up,down");
	...
	{
		scorehud.Clear();
		scorehud.AddLine(EMPTY, "Score: " + std::to_string(scene->getScore()));
	}
	...
	void PinballInput(int key)
	{
		switch (toupper(key))
		{
		case ' ':
			scene->ActivatePlunger();
			break;
		case 'Z':
			scene->ActivatePaddle(0);
			break;
		case 'X':
			scene->ActivatePaddle(1);
			break;
		default:
			break;
		}
